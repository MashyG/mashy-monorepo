...
// 遍历 stageOne 中的 headers 并将 headers 转换为对象
const transformHeaders2Obj = (headers: Headers) => {
  const headersObj = {}
...
async function getResponse(request, config) {
  let stageOne
  try {
    stageOne = await (await fetch(request)).clone()
  } catch (e) {
    return createError(
      'Network Error',
      config,
  ...
  const response: AxiosResponse = {
    ...
    headers: transformHeaders2Obj(stageOne.headers), // Make a copy of headers
    config: config,
    request,
  }
